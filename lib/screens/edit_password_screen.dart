...
class _EditPasswordScreenState extends State<EditPasswordScreen> {
  ...
  //
  //           /// decrypt password
  //           cryptor.decrypt(blob).then((value) {
  //             if (_isBip39Valid) {
  //               final mnemonic = bip39.entropyToMnemonic(value);
  //
  //               _passwordTextController.text = mnemonic;
  //               _initialPassword = mnemonic;
  //             } else {
  ...
  //
  //             setState(() {
  //               _passwordStrength = estimatePasswordStrength(value);
  //             });
  //
  //             _validateFields();
  //           });
  //         } else {
  //           if (geolocationManager.geoLocationUpdate != null) {
  //             decryptGeoLock(geolocationManager.geoLocationUpdate!)
  //                 .then((value) {
  //               if (value.isNotEmpty) {
  //                 logManager.logger.d("geo locked decrypted successfully");
  //
  ...
  //
  //         /// decrypt username
  //         cryptor.decrypt(username).then((value) {
  //           username = value;
  //
  //           _validateFields();
  //         });
  ...
  //           '$name.$username.${tags.toString()}.$isFavorite.$_initialPassword.$value';
  //
  //           qrItem = QRCodeItem(
  //               name: name, username: username, password: _initialPassword);
  //
  //           setState(() {
  ...
  //             // print("DECYRYPTING PREV PASS: ${pp.password} : ${pp.cdate}");
  //
  ...
  Future<GeoLockItem?> doGeoLockEncryption(String password) async {
    GeoLockItem? geoItem;

    final lat = geolocationManager.geoLocationUpdate?.userLocation.latitude;
    final long =
        geolocationManager.geoLocationUpdate?.userLocation.longitude;
